...
fs.copyFile('./assets/blogTemplate.html', `./blog/index.html`, err => {
	...
	jsdom.fromFile(`./blog/index.html`, options).then(dom => {
		...
		document.getElementsByTagName('title')[0].textContent = `${settingsJson[0].blogName}'s Blog` ? `${settingsJson[0].blogName}'s Blog` : 'Name of the Blog';
		document.getElementById('blog_title').textContent = `${settingsJson[0].blogName}'s Blog` ? `${settingsJson[0].blogName}'s Blog` : 'Name of the Blog';
		document.getElementById('blogIcon').setAttribute('href', settingsJson[0].blogIcon ? settingsJson[0].blogIcon : 'https://i.imgur.com/TFle1Cb.png');
